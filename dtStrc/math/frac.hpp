...
namespace elsie{
  ...
  public:
    ...
    template<std::unsigned_integral T>frac(T N,T D=1):s(0),n(N),d(D){}
    template<std::signed_integral T>frac(T N,T D=1){
      if(D==0)D=1;
    ...
    bool operator<=(const frac&o){return!(*this>o);}
    bool operator>(const frac&o){
      ul g=std::gcd(this->d,o.d),l=this->n*o.d/g,r=this->d/g*o.n;
    ...
    friend frac operator*(const frac&lhs,const frac&rhs){ return frac(lhs)*=rhs;}
    friend frac operator/(const frac&lhs,const frac&rhs){ return frac(lhs)/=rhs;}
    friend std::ostream&operator<<(std::ostream&dst,const frac&o){
      if(o.s)dst<<"-";
